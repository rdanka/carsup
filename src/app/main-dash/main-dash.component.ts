...
export class MainDashComponent implements OnInit {
  ...
  ngOnInit() {
    this.mechanics$ = this.dataService.loadInitialData().pipe(
      map((mechanics) =>
        mechanics.map((mechanic: any) => {
          return { name: mechanic.name, isToggled: true };
        })
      ),
      tap((mechanics: any) =>
        mechanics.forEach((mechanic: any) =>
          this.toggledMechanics.push(mechanic.name)
        )
      )
    );
    this.employers$ = this.dataService.loadInitialData().pipe(
      map((mechanics) =>
        mechanics.map((mechanic: any) =>
          mechanic.employers.map((employer: any) => {
            return { name: employer.name, isToggled: true };
          })
        )
      ),
      ...
      map((employers: any[]) =>
        employers.filter(
          (employer, index) =>
            employers.findIndex(
              (e) =>
                e.name === employer.name && e.isToggled === employer.isToggled
            ) === index
        )
      ),
      tap((employers: any[]) =>
      ...
      tap((employers: any) =>
        employers.forEach((employer: any) =>
          this.toggledEmployers.push(employer.name)
        )
      )
    );
  ...
  onToggle(person: any, type: string) {
    ...
    /* this.dataService
      ...
      .pipe(
        map((mechanics) =>
          mechanics.filter((mechanic: any) =>
            this.toggledMechanics.includes(mechanic.name)
          )
        ),
        map((mechanics) =>
          mechanics.map((mechanic: any) =>
            mechanic.employers.filter((employer: any) =>
              this.toggledEmployers.includes(employer.name)
            )
          )
        ),
        map((mechanics) =>
          mechanics.map((employers: any) =>
            employers.reduce(
              (sum: number, employer: any) => employer.wageProfit + sum,
              0
            )
          )
        ),
    ...
    this.dataService
      ...
      .pipe(
        ...
        map((mechanics) =>
          mechanics.map((employers: any) =>
            employers.reduce(
              (sum: number, employer: any) => employer.partsProfit + sum,
              0
            )
          )
        ),
    ...
    this.dataService
      ...
      .pipe(
        ...
        map((mechanics) =>
          mechanics.map((employers: any) =>
            employers.reduce(
              (sum: number, employer: any) => employer.partsPurchasePrice + sum,
              0
            )
          )
        ),
    ...
    this.totalCars$ = this.dataService.loadInitialData().pipe(
      ...
      map((mechanics) =>
        mechanics.reduce(
          (sum: number, mechanic: any) => mechanic.totalCars + sum,
          0
        )
      )
    );
    ...
    this.totalHours$ = this.dataService.loadInitialData().pipe(
      ...
      map((mechanics) =>
        mechanics.reduce(
          (sum: number, mechanic: any) => mechanic.totalHours + sum,
          0
        )
      )
    );
    ...
    this.lostTime$ = this.dataService.loadInitialData().pipe(
      ...
      map((mechanics) =>
        mechanics.reduce(
          (sum: number, mechanic: any) => mechanic.lostTime + sum,
          0
        )
      )
    );
    ...
    this.totalIncome$ = this.dataService.loadInitialData().pipe(
      ...
      map((employers) =>
        employers.map((employer: any) => employer.totalIncome)
      ),
      map((totalIncomes) =>
        totalIncomes.reduce(
          (sum: number, totalIncome: number) => totalIncome + sum,
          0
        )
      ),
